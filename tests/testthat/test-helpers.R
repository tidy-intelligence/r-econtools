...
test_that(
  ...
  ),
  {
    ...
    # E1 keeps year == y
    expect_true(any(res$entity_id == "E1" & res$year == y))
    # E2 had a future (y+1) which is dropped; most recent is y-1
    expect_true(any(res$entity_id == "E2" & res$year == (y - 1)))
...
test_that(
  ...
  ),
  {
    ...
    expect_equal(
      nrow(subset(res_tie, entity_id == "A")),
      2L
    )
    # For B, single max row retained
    expect_equal(
      nrow(subset(res_tie, entity_id == "B")),
      1L
    )
  }
)
